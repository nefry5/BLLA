...
const CAT_UNLOCK=[0,6,13,9,14,10,12,11,16,15,3,17];
const CAT_RANK_UNLOCKS:{[k:number]:number[]}={3:[10],6:[1],9:[3],10:[5],11:[7],12:[6],13:[2],14:[4],15:[9],16:[8],17:[11]};
const getUnlockedCatsForLevel=(levelIdx:number)=>CAT_RANK_UNLOCKS[levelIdx]||[];
const getUnlockLabelForLevel=(levelIdx:number)=>{const cats=getUnlockedCatsForLevel(levelIdx);return cats.length?cats.map(ci=>CAT_NAMES[ci]).join(", "):null;};
...
function ProgressHeader({ls,T}){const{rank,xIn,xNeed,isMax,pct,nextRank}=getRankInfo(ls.xp||0);const segs=20,filled=Math.round(pct/100*segs);return(<div style={{padding:"8px 12px",background:"rgba(0,0,0,0.35)",borderBottom:`1px solid ${T.brd}`,flexShrink:0}}><div style={{display:"flex",alignItems:"center",gap:12}}><RankBadge rank={rank} size={54}/><div style={{flex:1,minWidth:0}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}><span style={{color:rank.col,fontSize:13,fontWeight:900,textShadow:`0 0 10px ${rank.col}88`}}>{rank.l}</span><span style={{color:"rgba(255,255,255,.5)",fontSize:10,fontWeight:700}}>{isMax?"MAX ∞":`${xIn} / ${xNeed} XP`}</span></div><div style={{display:"flex",gap:2,padding:"2px",background:"rgba(0,0,0,0.3)",borderRadius:6}}>{Array.from({length:segs}).map((_,i)=>(<div key={i} style={{flex:1,height:10,borderRadius:2,background:i<filled?rank.col:"rgba(255,255,255,0.05)",boxShadow:i<filled?`0 0 4px ${rank.glow}`:"none",transition:"background .4s"}}/>))}</div></div>{nextRank&&<RankBadge rank={nextRank} size={28} opacity={.35}/>}<div style={{display:"flex",alignItems:"center",gap:3,flexShrink:0}}><CoinIcon size={14}/><span style={{color:"#ffd700",fontSize:11,fontWeight:700}}>{fmtCoins(ls.coins||0)}</span></div></div></div>);}
...
function RouletteOpening({box,wonItem,onClose,alreadyOwned=false}){
  ...
  useEffect(()=>{
    SFX.loot();
    const tickInt=setInterval(()=>SFX.rTick(.22+Math.random()*.35),88);
    ...
    // Reveal after settle
    const t4=setTimeout(()=>{clearInterval(tickInt);const rarSfx:{[k:string]:()=>void}={commun:SFX.lootC,rare:SFX.lootR,super_rare:SFX.lootSR,epique:SFX.lootE,legendaire:SFX.lootL,secret:SFX.lootS};(rarSfx[wonItem.rar]||SFX.lootC)();setPhase("reveal");},4350);
    return()=>{clearInterval(tickInt);clearTimeout(t1);clearTimeout(t2);clearTimeout(t3);clearTimeout(t4);};
  },[]);
...
function MilestonePopup({count,onClose}){useEffect(()=>{SFX.milestone();const t=setTimeout(onClose,2200);return()=>clearTimeout(t);},[]);const t=Math.min(1,count/1000);const col=t<.1?"#f39c12":t<.3?"#e67e22":t<.6?"#e74c3c":"#9b59b6";const col2=t<.1?"#ffe082":t<.3?"#ffb74d":t<.6?"#ff8a65":"#d1a3ff";return(<div style={{position:"absolute",top:"12%",left:"50%",transform:"translateX(-50%)",zIndex:150,pointerEvents:"none",animation:"fadeIn .2s ease"}}><div style={{minWidth:180,background:"linear-gradient(135deg,rgba(12,4,2,.92),rgba(35,10,6,.88))",border:`1.5px solid ${col}99`,borderRadius:18,padding:"10px 14px",textAlign:"center",boxShadow:`0 0 24px ${col}55, inset 0 0 18px rgba(255,255,255,.06)`,backdropFilter:"blur(6px)",animation:"popIn .35s cubic-bezier(.2,1.4,.4,1)"}}><div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:3}}><span style={{fontSize:24,lineHeight:1,filter:`drop-shadow(0 0 10px ${col})`}}>🔥</span><span style={{fontSize:18,fontWeight:900,background:`linear-gradient(135deg,${col2},${col})`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",letterSpacing:-.3}}>{M_MSG[count]||`${count} streak !`}</span></div><div style={{color:"rgba(255,255,255,.72)",fontSize:11,fontWeight:700}}>Série actuelle <span style={{color:col,fontSize:13}}>{count}</span></div></div></div>);}
...
function RankUpPopup({rank,coinsAwarded=0,onClose}:{rank:any,coinsAwarded?:number,onClose:()=>void}){const unlockTxt=getUnlockLabelForLevel(RANK_LEVELS.findIndex(r=>r.id===rank.id));useEffect(()=>{SFX.rankup();},[]);return(<div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.82)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:160,padding:20}} onClick={onClose}><div style={{background:`linear-gradient(135deg,#080012,#120020)`,border:`2px solid ${rank.col}`,borderRadius:26,padding:"28px 24px",textAlign:"center",boxShadow:`0 0 80px ${rank.col}88,0 0 160px ${rank.col}33`,animation:"popIn .5s cubic-bezier(.2,1.4,.4,1)",maxWidth:300,width:"100%"}} onClick={e=>e.stopPropagation()}><div style={{fontSize:12,color:"rgba(255,255,255,.4)",letterSpacing:2,textTransform:"uppercase",marginBottom:12}}>Promotion de rang !</div><div style={{display:"flex",justifyContent:"center",marginBottom:12}}><RankBadge rank={rank} size={72}/></div><div style={{fontSize:22,fontWeight:900,color:rank.col,marginBottom:4,textShadow:`0 0 20px ${rank.col}`}}>{rank.l}</div>{unlockTxt&&<div style={{margin:"4px auto 10px",padding:"6px 12px",width:"fit-content",borderRadius:10,background:`${rank.col}18`,border:`1px solid ${rank.col}44`,color:"rgba(255,255,255,.85)",fontSize:12,fontWeight:700}}>Nouvelle categorie : {unlockTxt}</div>}{coinsAwarded>0&&<div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:6,background:"rgba(255,215,0,0.1)",border:"1px solid rgba(255,215,0,0.25)",borderRadius:10,padding:"6px 14px",margin:"10px auto",width:"fit-content"}}><CoinIcon size={14}/><span style={{color:"#ffd700",fontWeight:800,fontSize:14}}>+{fmtCoins(coinsAwarded)}</span></div>}<button onClick={onClose} style={{marginTop:14,background:`linear-gradient(135deg,${rank.col},${rank.col}99)`,color:"#000",border:"none",borderRadius:12,padding:"10px 28px",cursor:"pointer",fontWeight:800,fontSize:14}}>Super ! 🎉</button></div></div>);}
...
function LessonsTab({words,rs,langKey,T,levelIdx=0}){
  ...
  const groups=useMemo(()=>{const g={};words.forEach(([w,fr])=>{const ci=catFn(w);if(!g[ci])g[ci]=[];g[ci].push([w,fr]);});Object.values(g).forEach(arr=>arr.sort((a,b)=>{const ra=rs[a[0]],rb=rs[b[0]];const ha=ra!=null,hb=rb!=null;if(!ha&&!hb)return 0;if(!ha)return 1;if(!hb)return -1;return ra-rb;}));return g;},[words,rs,catFn]);
  const catOrder=Object.keys(groups).map(Number).sort((a,b)=>a-b);
...
function QuizTab({words,rs,langKey,T,skinKey,onScore,onMenu,onQP,isActive=true,curXp=0,levelIdx=0}){
  ...
  const curCat=CAT_FNS[langKey](curWord[0]);
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",padding:"8px 12px",gap:8,position:"relative",zIndex:1}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
        ...
        <div style={{marginLeft:"auto",position:"relative",display:"flex",alignItems:"center"}}>
          {xpAnim&&<div style={{position:"fixed",top:"18%",left:"50%",transform:"translateX(-50%)",display:"flex",flexDirection:"column",alignItems:"center",gap:3,pointerEvents:"none",zIndex:50}}><span style={{color:"#4ecdc4",fontSize:18,fontWeight:900,animation:"floatUp 1s ease forwards",textShadow:"0 0 12px #4ecdc4",letterSpacing:-.5}}>+{xpAnim.base} XP</span>{xpAnim.bonus>0&&<span style={{color:"#ff9800",fontSize:16,fontWeight:900,animation:"floatUp 1s ease .12s forwards",textShadow:"0 0 10px #ff9800"}}>+{xpAnim.bonus} XP 🔥</span>}</div>}
          <Flame streak={streak}/>
        </div>
      </div>
      <div style={{display:"flex",justifyContent:"center",marginTop:-2,marginBottom:2,flexShrink:0}}><div style={{background:"rgba(255,255,255,.06)",border:`1px solid ${T.brd}`,borderRadius:999,padding:"4px 10px",color:T.acc,fontSize:11,fontWeight:800,letterSpacing:.3}}>{CAT_ICONS[curCat]||"🌀"} {CAT_NAMES[curCat]||"Divers"}</div></div>
      {/* Score */}
...
// ── PROFILE ───────────────────────────────────────────────────────────────
function ProfileScreen({state,T,onBack,onChangePseudo,debugCoins,onUnlockAll,onResetLang,onMasterAll,onSetRank,currentLang}){
          ...
          <div style={{background:"linear-gradient(135deg,#0d0d1a,#1a0a2e)",border:`1px solid ${T.brd}`,borderRadius:22,padding:"20px 16px",maxWidth:320,width:"100%",maxHeight:"80vh",overflowY:"auto"}}>
            ...
            {RANK_LEVELS.map((r,idx)=>(
              <div key={r.id} style={{display:"flex",alignItems:"center",gap:14,padding:"10px 12px",background:"rgba(255,255,255,.04)",border:`1px solid ${r.col}33`,borderRadius:12,marginBottom:8}}>
                ...
                <div style={{flex:1}}><div style={{color:r.col,fontWeight:900,fontSize:15,textShadow:`0 0 8px ${r.col}88`}}>{r.l}</div><div style={{color:T.sub,fontSize:11,marginTop:2}}>{r.cumXP===0?"Départ":r.cumXP.toLocaleString()+" XP cumulés"}</div>{getUnlockLabelForLevel(idx)&&<div style={{color:"rgba(255,255,255,.55)",fontSize:10,marginTop:3}}>Debloque : {getUnlockLabelForLevel(idx)}</div>}</div>
                {currentLang&&<button onClick={()=>onSetRank(currentLang,r)} style={{background:`linear-gradient(135deg,${r.col},${r.col}88)`,border:"none",color:"#000",borderRadius:9,padding:"6px 10px",cursor:"pointer",fontSize:10,fontWeight:900,flexShrink:0}}>Avoir</button>}
              </div>
            ))}
          </div>
        </div>
...
function HomeScreen({state,T,onSelect,onProfile,onLeaderboard}){
  return(<div style={{minHeight:"100%",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:`linear-gradient(135deg,${T.bg},${T.surf})`,padding:20,fontFamily:"'Segoe UI',sans-serif",position:"relative"}}>
    ...
    <h1 style={{color:T.txt,fontSize:24,margin:"0 0 4px",fontWeight:800,letterSpacing:".2px",textAlign:"center"}}>🌍 Basic Languages</h1>
    ...
    <div style={{display:"flex",gap:12,flexWrap:"wrap",justifyContent:"center",width:"100%",maxWidth:420}}>
      {Object.entries(LANG_INFO).map(([lk,li])=>{const ls=state.langs[lk];const ws=WORDS[lk],tot=ws.length;const m=ws.filter(([w])=>(ls.rawScores[w]??-1)>=70).length;const{rank}=getRankInfo(ls.xp||0);
      return(<button key={lk} onClick={()=>{SFX.nav();onSelect(lk);}} style={{background:"rgba(255,255,255,.05)",border:`1px solid ${T.brd}`,borderRadius:20,padding:"16px 0",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:8,flex:"1 1 140px",maxWidth:195,transition:"all .2s",position:"relative"}} onMouseEnter={e=>{e.currentTarget.style.background="rgba(255,255,255,.1)";e.currentTarget.style.transform="translateY(-4px)";}} onMouseLeave={e=>{e.currentTarget.style.background="rgba(255,255,255,.05)";e.currentTarget.style.transform="";}}>
        ...
        <div style={{width:"78%"}}><div style={{display:"flex",gap:1.5,marginBottom:3}}>{Array.from({length:12}).map((_,i)=>{const f=i<Math.round((m/tot)*12);return(<div key={i} style={{flex:1,height:5,borderRadius:1,background:f?T.acc:"rgba(255,255,255,.07)",boxShadow:f?`0 0 3px ${T.acc}`:"none"}}/>);})}</div><div style={{color:T.sub,fontSize:9,textAlign:"center"}}>{m}/{tot} · {Math.round(m/tot*100)}%</div></div>
...
export default function App(){
  ...
  function handleSetRank(lk:string,rank:any){setState((prev:any)=>{const l=prev.langs[lk];const nextXp=rank.cumXP;return{...prev,langs:{...prev.langs,[lk]:{...l,xp:nextXp}}};});}
